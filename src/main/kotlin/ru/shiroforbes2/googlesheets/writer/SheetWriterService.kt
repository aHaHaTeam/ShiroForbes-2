...
class SheetWriterService(
  ...
  fun updateTransactions(
    ...
    val table: ComposedTable = tableComposer.composeTable(group, transactions)
    ...
    val sheetId =
      spreadsheet.sheets
        ...
        ?.properties
        ?.sheetId
        ?: createSheet(spreadsheetId, sheetTitle, height = table.cells.size)

    clearSheet(spreadsheet.spreadsheetId, sheetTitle)

    batchUpdate(
      spreadsheetId,
      table.generateUpdateRequests(sheetId),
      "Failed to update transactions (spreadsheetId=[$spreadsheetId], sheetTitle=[$sheetTitle])",
    )
  }
